...
class HTTPClient:
    ...
    async def initialise_keys(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            resp = await session.post("https://developer.clashroyale.com/api/login", json=body)
            if resp.status == 403:
            ...
            resp = await session.post("https://developer.clashroyale.com/api/apikey/list")
            ...
            if len(self._keys) < self.key_count:
                for key in keys[:]:
                    ...
                    resp = await session.post("https://developer.clashroyale.com/api/apikey/revoke", json={"id": key["id"]})
                    if resp.status == 200:
                        keys.remove(key)
                ...
                while len(self._keys) < self.key_count and len(keys) < KEY_MAXIMUM:
                    ...
                    resp = await session.post("https://developer.clashroyale.com/api/apikey/create", json=data)
                    key = await resp.json()
            ...
            if len(self._keys) == 0:
                ...
                raise RuntimeError(
                        ...
                        "Please specify a key_name kwarg, or go to 'https://developer.clashroyale.com' to delete "
