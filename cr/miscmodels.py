...
class BaseArena:
    """Represents a basic arena.

    Attributes
    -----------
    id: :class:`int`: The arena's unique ID
    name: :class:`str`: The arena's name, as it appears in-game."""
...
class Arena(BaseArena):
    ...
    -----------
    id:
        :class:`int`: The arena ID.
    name:
        :class:`str`: The arena name.
    icon:
        :class:`Icon`: The arena's icon.
    """
...
class Progress:
    """Represents a Clash Royal Player progress."""
    __slots__ = ("arena", "trophies", "best_trophies", "name", "_client", "arena_cls")
    
    def __init__(self, *, data, client, arena_cls: BaseArena = None, name: Optional[str] = None):
        self._client = client
        if not arena_cls:
            arena_cls = client.objects_cls.get('BaseArena', client.objects_cls.get('Arena', BaseArena))
        self.arena_cls = arena_cls
        self.arena: BaseArena = try_enum(arena_cls, data=data.get("arena"), client=self._client)
        self.trophies: Optional[int] = data.get("trophies")
        self.best_trophies: Optional[int] = data.get("bestTrophies")
        if not name:
            name = data.get("name")
...
class SeasonResult:
    """Represents a Clash Royale Season Result (Path of Legend Season Result)."""
    __slots__ = ("trophies", "rank", "league_number", "_client")
    ...
    def __init__(self, *, data, client):
        ...
        self.league_number: Optional[int] = data.get("leagueNumber")
...
class LeagueStatistics:
    """Represents the League Statistics for a player."""
    def __init__(self, *, data, client):
        self._client = client
